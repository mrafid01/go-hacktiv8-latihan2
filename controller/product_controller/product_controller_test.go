...
// latihan delete success (positif)
func TestProductController_DeleteProduct_Success(t *testing.T) {
	...
	req := httptest.NewRequest(http.MethodDelete, "/products/1", bytes.NewBufferString(""))
...
// latihan delete gagal (negatif)
func TestProductController_DeleteProduct_InvalidProductIDParams(t *testing.T) {
	product_service.ProductService = &productServiceMock{}

	deleteProduct = func(int64) error_utils.MessageErr {
		return error_utils.NewBadRequest("invalid product id params")
	}

	req := httptest.NewRequest(http.MethodDelete, "/products/asd", bytes.NewBufferString(""))
